...
def GenerateMeshes(shape_parameters,test_shapes,plot_airfoils):
    ...
    for i in range(len(shape_parameters)):    
        ...
        airfoil_mesh_parameters["solver_settings"]["model_import_settings"]["input_filename"].SetString("naca0012_3D_0aoa_00")
        ...
        if (i>(len(shape_parameters)-test_shapes-1)):
            ...
            fig = plt.plot( xp, zp, "+", markersize=5, label = "Train Airfoil "+str(i)+" t="+str(np.round(shape_parameters[i,0],3))+" m="+str(np.round(shape_parameters[i,1],3))+" p="+str(np.round(shape_parameters[i,2],3)))
...
def GetRomManagerParameters():
    general_rom_manager_parameters = KratosMultiphysics.Parameters("""{
            ...
            "ROM":{
                ...
                "rom_basis_output_name": "ROM_Basis/RomParameters",
...
if __name__ == "__main__":
    ...
    rom_manager = RomManager(project_parameters_name,general_rom_manager_parameters,CustomizeSimulation,UpdateProjectParameters,"Primal")

    rom_manager.Fit(mu_train,NumberOfTrainShapes) 

    rom_manager.Test(mu_test,NumberOfTestShapes) 
