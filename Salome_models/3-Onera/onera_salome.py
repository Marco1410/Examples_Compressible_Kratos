...
#### WAKE
wake_stl = geompy.MakePrismDXDYDZ(TE_Wing, Wake_length, 0, 0)
aux_wake_stl = geompy.MakePrismDXDYDZ(TE_Wing, -0.1, 0, 0)
TE_Wake = geompy.CreateGroup(wake_stl, geompy.ShapeType["EDGE"])
geompy.UnionIDs(TE_Wake, [9])
geompy.Rotate(wake_stl, TE_Wing, Wake_angle*math.pi/180.0)
geompy.Rotate(aux_wake_stl, TE_Wing, -WingAngle*math.pi/180.0)
wake = geompy.MakeFuseList([wake_stl, aux_wake_stl], True, True)
##########################################################################
...
geompy.addToStudy( aux_wake_stl, 'aux_wake_stl' )
