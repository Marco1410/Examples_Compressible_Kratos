...
class CustomRomManager(object):
    ...
    def __LaunchTrainROM(self, mu_train):
        ...
        SnapshotsMatrix = []
        DataBase_folder = "../TransonicDataBase/DataBase/"
        parameters_copy = self.UpdateProjectParameters(parameters.Clone(), mu_train[0])
        parameters_copy = self._AddBasisCreationToProjectParameters(parameters_copy)
        parameters_copy["solver_settings"]["maximum_iterations"].SetInt(1)
        ...
        for process in simulation._GetListOfOutputProcesses():
            if isinstance(process, CalculateRomBasisOutputProcess):
                self.BasisOutputProcess = process
        # for mu in mu_train:
        #     SnapshotsMatrix.append(self.GetSnapshotsMatrixFromDataBase(DataBase_folder+mu[3])) 
        # SnapshotsMatrix = np.block(SnapshotsMatrix)
        # self.BasisOutputProcess._PrintRomBasis(SnapshotsMatrix)

        SnapshotsMatrix = np.load("full_fom_snapshots.npy")
        SnapshotsMatrix = np.block(SnapshotsMatrix)
        self.BasisOutputProcess._PrintRomBasis(SnapshotsMatrix)
    ...
    def GetSnapshotsMatrixFromDataBase(self,path):
        archivo = open(path, 'rb')
        snapshots_data_list = pickle.load(archivo)
        archivo.close()
        snapshots_matrix = np.empty((self.BasisOutputProcess.n_nodal_unknowns*self.BasisOutputProcess.n_nodes,self.BasisOutputProcess.n_data_cols))
        for i_col in range(self.BasisOutputProcess.n_data_cols):
            aux_col = np.array(snapshots_data_list[i_col]).reshape(self.BasisOutputProcess.n_nodal_unknowns*self.BasisOutputProcess.n_nodes,
                                                                   self.BasisOutputProcess.n_data_cols)
    ...
    def __LaunchTestFOM(self, mu_test):
        ...
        SnapshotsMatrix = []
        DataBase_folder = "../TransonicDataBase/DataBase/"
        parameters_copy = self.UpdateProjectParameters(parameters.Clone(), mu_test[0])
        parameters_copy = self._AddBasisCreationToProjectParameters(parameters_copy)
        parameters_copy["solver_settings"]["maximum_iterations"].SetInt(1)
        parameters_copy["output_processes"].RemoveValue("gid_output")
        model = KratosMultiphysics.Model()
    ...
    def _StoreResultsByName(self,parameters,results_name,mu, Id):
        ...
        if self.general_rom_manager_parameters["save_gid_output"].GetBool():
            parameters["output_processes"]["gid_output"][0]["Parameters"]["output_name"].SetString('Results/'+ results_name + "_" +  case_name)
        else:
            parameters["output_processes"].RemoveValue("gid_output")
        if self.general_rom_manager_parameters["save_vtk_output"].GetBool():
            parameters["output_processes"]["vtk_output"][0]["Parameters"]["output_path"].SetString('Results/vtk_output_'+ results_name + "_" + case_name)
        else:
