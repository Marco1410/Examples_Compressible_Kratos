...
class CustomRomManager(object):
    ...
    def __LaunchTrainROM(self, mu_train):
        ...
        BasisOutputProcess = CalculateRomBasisOutputProcess(model,parameters_copy["output_processes"]["rom_output"][0]["Parameters"])
        SnapshotsMatrix = np.block(np.load("rom_data/SnapshotsMatrices/full_fom_snapshots.npy"))
        # BasisOutputProcess._PrintRomBasis(SnapshotsMatrix) 

        SnapshotsMatrixError = np.empty((68258, 0))

        columns = np.arange(0,389,1)
        columns_d= np.arange(0,389,13)
        np.delete(columns,columns_d,axis=0)

        for i in columns:
            SnapshotsMatrixError = np.append(SnapshotsMatrixError, SnapshotsMatrix[:][i], axis=1)
        SnapshotsMatrixError = np.block(SnapshotsMatrixError)

        print("matrix error 2 :", SnapshotsMatrixError.shape)
        print("matrix error 2 :", SnapshotsMatrixError)
        input("pause ::::::::::::::::::::::::::::::")
