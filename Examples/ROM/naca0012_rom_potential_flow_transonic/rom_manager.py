...
def get_multiple_params_by_Halton_train():
    angles = np.arange( 0.00, 1.51 , 0.25)
    machs  = np.arange( 0.60, 0.81, 0.02)
    mu = []
    for i in range(len(angles)):
        for j in range(len(machs)):
            #Angle of attack , Mach infinit, Upwind factor constant
            mu.append([np.round(angles[i] * np.pi / 180.0,3), np.round(machs[j],3), np.round(1.000,3)])
    return mu
...
def select_upwind_factor_constant(mu):
    ...
    for n in range(len(mu)):
        ...
        while (convergence_ratio > tolerancia and absolute_norm > tolerancia):
            ...
            parameters["processes"]["boundary_conditions_process_list"][0]["Parameters"]["mach_infinity"].SetDouble(mach_infinity)
            parameters["processes"]["boundary_conditions_process_list"][0]["Parameters"]["critical_mach"].SetDouble(critical_mach)
            # parameters["output_processes"]["gid_output"][0]["Parameters"]["output_name"].SetString("GiD_Upwind_search/Simulation_" + "A_"+str(angle_of_attack*180/np.pi) + "_M_" + str(mach_infinity))
...
if __name__ == "__main__":
    ...
    mu_train = get_multiple_params_by_Halton_train()
