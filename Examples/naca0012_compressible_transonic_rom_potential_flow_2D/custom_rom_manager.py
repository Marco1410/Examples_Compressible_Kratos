...
# 
# Podría haber un custom_ (fom/rom/hrom)
# 

class CustomRomManager(object):

    def __init__(self,project_parameters_name, general_rom_manager_parameters, CustomizeSimulation, UpdateProjectParameters,UpdateMaterialParametersFile, CustomizeSimulation2):
        ...
        self.CustomizeSimulation2 = CustomizeSimulation2
    ...
    def __LaunchTrainROM(self, mu_train):
        ...
        for Id, mu in enumerate(mu_train):
            ...
            while (convergence_ratio > tolerancia and absolute_norm > tolerancia):
                ...
                parameters_copy["solver_settings"]["solving_strategy_settings"]["advanced_settings"]["max_alpha"].SetDouble(2.0)
                ...
                upwind_factor_constant = parameters_copy["processes"]["boundary_conditions_process_list"][0]["Parameters"]["upwind_factor_constant"].GetDouble()
                if convergence_ratio >= 0.5:
                    upwind_factor_constant += 0.2
                else:
                    upwind_factor_constant += 0.05
                    parameters_copy["solver_settings"]["maximum_iterations"].SetInt(300)
                parameters_copy["processes"]["boundary_conditions_process_list"][0]["Parameters"]["upwind_factor_constant"].SetDouble(upwind_factor_constant)
            ...
            SnapshotsMatrixError.append(BasisOutputProcess._GetSnapshotsMatrix())
            self._StoreSnapshotsMatrix('trainStepSolution_'+str(mu[0])+"_"+str(mu[1])+"_"+str(np.round(mu[2],2)), SnapshotsMatrix)

            tolerances_lists = [1e-10,1e-9,1e-8,1e-7,1e-6,1e-5,5e-10,5e-9,5e-8,5e-7,5e-6,5e-5]
            count = 0
            for tolerance in tolerances_lists:
                ...
                model = KratosMultiphysics.Model()
                analysis_stage_class = self._GetAnalysisStageClass(parameters_copy)
                simulation = self.CustomizeSimulation2(analysis_stage_class,model,parameters_copy)
                simulation.Run()
                ...
                for process in simulation._GetListOfOutputProcesses():
                    if isinstance(process, CalculateRomBasisOutputProcess):
                        BasisOutputProcess = process
                SnapshotsMatrix.append(BasisOutputProcess._GetSnapshotsMatrix())
                self._StoreSnapshotsMatrix('trainStepSolution_'+str(mu[0])+"_"+str(mu[1])+"_"+str(np.round(mu[2],2))+"_"+str(count), SnapshotsMatrix)

        SnapshotsMatrix = np.block(SnapshotsMatrix)
        SnapshotsMatrixError = np.block(SnapshotsMatrixError)
        
        BasisOutputProcess._PrintRomBasis(SnapshotsMatrix) 

        self._StoreSnapshotsMatrix('full_fom_snapshots', SnapshotsMatrix)

        return SnapshotsMatrixError
        ...
        for Id, mu in enumerate(mu_test):
            ...
            for process in simulation._GetListOfOutputProcesses():
                if isinstance(process, CalculateRomBasisOutputProcess):
                    BasisOutputProcess = process
            SnapshotsMatrix.append(BasisOutputProcess._GetSnapshotsMatrix()) 
            self._StoreSnapshotsMatrix('testStepSolution_'+str(mu[0])+"_"+str(mu[1])+"_"+str(np.round(mu[2],2)), SnapshotsMatrix)

        SnapshotsMatrix = np.block(SnapshotsMatrix)
        self._StoreSnapshotsMatrix('full_fom_test_snapshots', SnapshotsMatrix)
