...
def upwind(x,y): #0.0 0.6 / 5.0 0.85
    return 2.1489732 /(1 + np.exp(-(1.55470776*x+37.876707*y-31.48703546))) + 0.09767296
...
if __name__ == "__main__":
    ...
    mach_infinity   = 0.75
    angle_of_attack = -0.025 + 2.21
    critical_mach   = transonic_parameters(mach_infinity)
    mach_number_limit      = np.sqrt(3)

    upwind_factor_constant = upwind(angle_of_attack,mach_infinity)
    ...
    while (convergence_ratio > 1e-10 and absolute_norm > 1e-10):
        ...
        absolute_norm     = global_model["FluidModelPart"].ProcessInfo[KratosMultiphysics.RESIDUAL_NORM]
