...
def upwind_aprox(angle,mach):
    ...
    angle_data  = np.loadtxt("parameters_list.dat",usecols=(0,))
    mach_data   = np.loadtxt("parameters_list.dat",usecols=(1,))
    upwind_data = np.loadtxt("parameters_list.dat",usecols=(2,))
    # Fit a radial basis function model        
    # 'multiquadric','inverse','gaussian','linear'
    # 'cubic','quintic','thin_plate'
    rbf = Rbf(angle_data, mach_data, upwind_data, function="multiquadric", epsilon=0.2, smooth=0.5)
    return rbf(angle, mach)
...
if __name__ == "__main__":
    ...
    machs  = np.arange( 0.60, 0.851, 0.05)
    ...
    for angle_of_attack in angles:
        ...
        # Reset the upwind constant for each angle
        ...
        for mach_infinity in machs:
            ...
            while (convergence_ratio > tolerancia and absolute_norm > tolerancia):
                ...
                if os.path.isfile("parameters_list.dat") and use_aprox == True:
            ...
            if write_and_plot:
                ...
                parameters_list.append([ angle_of_attack, mach_infinity, upwind_factor_constant])
    ...
    KratosMultiphysics.kratos_utilities.DeleteFileIfExisting('parameters_list.dat')
    alpha  = np.zeros(len(parameters_list))
    mach   = np.zeros(len(parameters_list))
    upwind = np.zeros(len(parameters_list))
    fout=open("parameters_list.dat",'w')
    for i in range(len(parameters_list)):
        alpha[i]  = parameters_list[i][0]
        mach[i]   = parameters_list[i][1]
        upwind[i] = parameters_list[i][2]
        fout.write("%s %s %s\n" %(alpha[i], mach[i], upwind[i]))
    fout.close()