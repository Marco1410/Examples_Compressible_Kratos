...
for j in range(len(mu_train)):
    ...
    for n, name in enumerate(case_names):
        if os.path.exists("Data/"+name+"_"+"train"+"_"+str(j)+".dat"):
            ...
            if np.max(cp) > cp_max:
                cp_max = np.max(cp)
    angle = np.double(mu_train[j][0])
    fig = plt.title('Cp vs x - ' + "angle: " + str(angle) + "º " + "mach: " + str(mu_train[j][1]))
    ...
    fig = plt.savefig("Captures/Simulation_train_" + str(j) + "_A_" + str(angle) + "_M_" + str(mu_train[j][1])+".png")
    fig = plt.close('all')
...
for j in range(len(mu_test)):
    ...
    for n, name in enumerate(case_names):
        if os.path.exists("Data/"+name+"_"+"test"+"_"+str(j)+".dat"):
            ...
            if np.max(cp) > cp_max:
                cp_max = np.max(cp)
    angle = np.double(mu_train[j][0])
    fig = plt.title('Cp vs x - ' + "angle: " + str(angle) + "º " + "mach: " + str(mu_test[j][1]))
    fig = plt.axis([-0.05,1.35,cp_max+0.1,cp_min-0.1])
    fig = plt.ylabel('Cp')
    fig = plt.xlabel('x')
    fig = plt.grid()
    fig = plt.legend()
    fig = plt.tight_layout()
    fig = plt.savefig("Captures/Simulation_test_" + str(j) + "_A_" + str(angle) + "_M_" + str(mu_test[j][1])+".png")
    fig = plt.close('all')