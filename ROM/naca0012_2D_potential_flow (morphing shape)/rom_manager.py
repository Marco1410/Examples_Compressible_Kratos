    ...
    class CustomSimulation(cls):
        ...
        def Initialize(self):
            ...
            for node in modelpart.Nodes:
                node.X0,node.Y0,node.Z0 = self.coordinates_new_shape(node.X,node.Y,node.Z)
                node.X ,node.Y ,node.Z  = self.coordinates_new_shape(node.X,node.Y,node.Z)
        ...
        def coordinates_new_shape(self,x,y,z):
            ...
            filename = "AirfoilsData/airfoil_parameters.dat"
...
if __name__ == "__main__":
    ...
    m_range    = [0.0 , 0.03] # Máxima curvatura
    p_range    = [0.25, 0.35] # Posicion de máxima curvatura
    shape_parameters = get_multiple_params_by_Halton(NumberOfShapes,tmax_range,m_range,p_range)
    ...
    rom_manager = RomManager(project_parameters_name,general_rom_manager_parameters,CustomizeSimulation,UpdateProjectParameters,"Primal",NumberOfShapes,shape_parameters)
