    ...
    for j in range(len(mu_train)):
        ...
        for n, name in enumerate(case_names):
            if os.path.exists("Data/"+name+"_"+"train"+"_"+str(j)+".dat"):
                ...
                if np.max(cp) > cp_max:
                    cp_max = np.max(cp)
        fig = plt.title('Cp vs x - ' + "angle: " + str(np.round(mu_train[j][0],3)) + "º " + "mach: " + str(np.round(mu_train[j][1],3)))
        ...
        fig = plt.savefig("Captures/Simulation_train_" + str(j) + "_A_" + str(np.round(mu_train[j][0],3)) + "_M_" + str(np.round(mu_train[j][1],3))+".png")
        fig = plt.close('all')
    ...
    for j in range(len(mu_test)):
        ...
        for n, name in enumerate(case_names):
            if os.path.exists("Data/"+name+"_"+"test"+"_"+str(j)+".dat"):
                ...
                if np.max(cp) > cp_max:
                    cp_max = np.max(cp)
        fig = plt.title('Cp vs x - ' + "angle: " + str(np.round(mu_test[j][0],3)) + "º " + "mach: " + str(np.round(mu_test[j][1],3)))
        fig = plt.axis([-0.05,1.35,cp_max+0.1,cp_min-0.1])
        fig = plt.ylabel('Cp')
        fig = plt.xlabel('x')
        fig = plt.grid()
        fig = plt.legend()
        fig = plt.tight_layout()
        fig = plt.savefig("Captures/Simulation_test_" + str(j) + "_A_" + str(np.round(mu_test[j][0],3)) + "_M_" + str(np.round(mu_test[j][1],3))+".png")
        fig = plt.close('all')
