    ...
    class CustomSimulation(cls):
        ...
        def FinalizeSolutionStep(self):
            ...
            if 'HHROM' in nametype:
                ...
                skin_data_filename = f"HHROM_Skin_Data/{mu[3]}/{simulation_name}.dat"
            else:
...
def FakeProjectParameters(parameters, mu):
    ...
    if mu[2] == 'HHROM':
        parameters["output_processes"]["gid_output"][0]["Parameters"]["output_name"].SetString(f'Results/{mu[3]}/{mu[2]}_{angle_of_attack}, {mach_infinity}')
        parameters["output_processes"]["vtk_output"][0]["Parameters"]["output_path"].SetString(f'Results/vtk_output_{mu[3]}/{mu[2]}_{angle_of_attack}, {mach_infinity}')
    else:
        parameters["output_processes"]["gid_output"][0]["Parameters"]["output_name"].SetString(f'Results/{mu[2]}_{angle_of_attack}, {mach_infinity}')
        parameters["output_processes"]["vtk_output"][0]["Parameters"]["output_path"].SetString(f'Results/vtk_output_{mu[2]}_{angle_of_attack}, {mach_infinity}')
    return parameters
...
if __name__ == "__main__":
    ...
    folder_names = ["Train_Captures", "Test_Captures", "Validation"]
    ...
    for id, constant, strategy in zip(n, constants, strategies):
        ...
        mu_test       = load_mu_parameters('Mu_history/mu_test')
        mu_validation = load_mu_parameters('Mu_history/mu_validation')

        Plot_Cps(mu_validation, 'Validation'    , f'{strategy}_{constant}', rbf_full=rbf_full)
        Plot_Cps(mu_test,       'Test_Captures' , f'{strategy}_{constant}', rbf_full=rbf_full)
        Plot_Cps(mu_train,      'Train_Captures', f'{strategy}_{constant}', rbf_full=rbf_full)