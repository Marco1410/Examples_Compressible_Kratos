    ...
    class CustomSimulation(cls):
        ...
        def __init__(self, model,project_parameters):
            ...
            if len(mu) > 0:
                ...
                self.case_subname = f'{constant[0]}_{constant[1]}'
            ...
            if parameters["output_processes"].Has("gid_output"):
                ...
                if 'FOM' in self.simulation_name:
                    ...
                    if case_type == 'train_fom': #Only for train FOM
                        ...
                        # Elements list for HROM approximation
                ...
                if os.path.exists(f'{case}_data.xlsx'):
                    ...
                    hoja = wb.active
                    hoja.append(('Strategy', 'Constant', 'Case name','Angle [º]', 'Mach', 'Residual norm', 'NL iter','u relative error', 'q relative error', 'Modes', 'Time [sec]', 'Nodes', 'Cl', 'Cd'))
                    for item in info_steps_list:
                        hoja.append(item)
                    wb.save(f'{case}_data.xlsx')
...
def evaluate_constant_lspg(constant):
    ...
    general_rom_manager_parameters["projection_strategy"].SetString('lspg')
    ...
    rom_manager = RomManager(project_parameters_name,general_rom_manager_parameters,
                            ...
                            rebuild_phi=True, rebuild_phiHROM=True, relaunch_TrainHROM=True, constant=[strategy,constant])

    rom_manager.Fit(load_mu_parameters('train'))
    rom_manager.Test(load_mu_parameters('test'))
    
    resume = []
    resume.append(['lspg', 'ROMvsFOM', constant, 
                   len(load_mu_parameters('train')), rom_manager.ROMvsFOM['Fit'],
                   len(load_mu_parameters('test')), rom_manager.ROMvsFOM['Test']])
...
def evaluate_constant_galerkin(constant):
    general_rom_manager_parameters = GetRomManagerParameters()
    project_parameters_name = "ProjectParameters.json"
    general_rom_manager_parameters["projection_strategy"].SetString('galerkin')
    ...
    rom_manager.Fit(mu_train=load_mu_parameters('train'))
    rom_manager.Test(load_mu_parameters('test'))

    resume = []
    resume.append(['galerkin', 'ROMvsFOM', constant, 
                   len(load_mu_parameters('train')), rom_manager.ROMvsFOM['Fit'],
                   len(load_mu_parameters('test')), rom_manager.ROMvsFOM['Test']])
    ...
    for strategy, optimal_constant in zip(strategies, optimal_constants):
        if optimal_constant < 0:
            ...
            if strategy=='galerkin':
                if not os.path.exists(f'Results/galerkin_1'):
                    evaluate_constant_galerkin(constant=1.0)
                result = minimize_scalar(evaluate_constant_galerkin, bounds=constant_range, method='bounded', tol=1e-9, options={'maxiter':10})
            else:
                if not os.path.exists(f'Results/lspg_1'):
                    evaluate_constant_lspg(constant=1.0)
                result = minimize_scalar(evaluate_constant_lspg, bounds=constant_range, method='bounded', tol=1e-9, options={'maxiter':10})
        ...
        if VALIDATION:
            ...
            for mu in mu_validation:
                ...
                if os.path.exists(f'HROM_Snapshots/{strategy}_{optimal_constant}/{mu[0]}, {mu[1]}.npy'):
                    run_hrom.append(np.load(f'HROM_Snapshots/{strategy}_{optimal_constant}/{mu[0]},{mu[1]}.npy'))
                if os.path.exists(f'HHROM_Snapshots/{strategy}_{optimal_constant}/{mu[0]}, {mu[1]}.npy'):
                    run_hhrom.append(np.load(f'HHROM_Snapshots/{strategy}_{optimal_constant}/{mu[0]}, {mu[1]}.npy'))
