...
def Plot_Cps(mu_list, capture_directory):
    ...
    for id, mu in enumerate(mu_list):
        ...
        cp_min = np.min([np.min(cp_hhrom), np.min(cp_hrom), np.min(cp_fom), np.min(cp_rom), np.min(cp_rbf), np.min(cp_validation), np.min(cp_xfoil)])
        cp_max = np.max([np.max(cp_hhrom), np.max(cp_hrom), np.max(cp_fom), np.max(cp_rom), np.max(cp_rbf), np.max(cp_validation), np.max(cp_xfoil)])
