...
if __name__ == "__main__":
    ...
    strategies         = ['galerkin']
    mach_range         = [0.69, 0.71]
    angle_range        = [0.50, 1.50]
    # mach_range         = [0.74, 0.75]
    ...
    if VALIDATION:
        if angle_range[0] <= 0.0 and angle_range[1]>= 0.0 and mach_range[0] <= 0.75 and mach_range[1]>=0.75:
            mu_validation.append([0.0,0.75])
    ...
    for strategy in strategies:
        ...
        for tolerance in tolerances_error:
            if mu_test==[]: break
            ...
            if len(new_mu_train)>0:
                ...
                plot_mu_values(mu_train, mu_test, mu_validation, 'i-MuValues')
                fit = True
            else:
                fit = False

        np.save('mu_train_final', mu_train)
        np.save('mu_train_not_scaled_final', mu_train_not_scaled)
        np.save('mu_test_final', mu_test)        
        np.save('mu_test_not_scaled_final', mu_test_not_scaled)        
        plot_mu_values(mu_train, mu_test, mu_validation, 'FinalMuValues')
