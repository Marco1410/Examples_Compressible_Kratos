...
if __name__ == "__main__":
    ...
    for n, mu in enumerate(mu_validation):
        ...
        if (mach_infinity == 0.75 and angle_of_attack == 0.7):
            # input('1')
            parameters["solver_settings"]["scheme_settings"]["initial_critical_mach"].SetDouble(0.88)
            parameters["solver_settings"]["scheme_settings"]["initial_upwind_factor_constant"].SetDouble(1.6)
            # parameters["solver_settings"]["scheme_settings"]["update_relative_residual_norm"].SetDouble(1e-3)
            parameters["solver_settings"]["scheme_settings"]["target_critical_mach"].SetDouble(0.88)
            parameters["solver_settings"]["scheme_settings"]["target_upwind_factor_constant"].SetDouble(1.6)

        if (mach_infinity == 0.7 and angle_of_attack == 1.00):
            # input('2')
            parameters["solver_settings"]["scheme_settings"]["initial_critical_mach"].SetDouble(0.99)
            parameters["solver_settings"]["scheme_settings"]["initial_upwind_factor_constant"].SetDouble(1.0)
            # parameters["solver_settings"]["scheme_settings"]["update_relative_residual_norm"].SetDouble(1e-3)
        ...
        if (mach_infinity >= 0.7 and angle_of_attack >= 2.00):
            # input('3')
            parameters["solver_settings"]["scheme_settings"]["initial_critical_mach"].SetDouble(0.93)
            parameters["solver_settings"]["scheme_settings"]["initial_upwind_factor_constant"].SetDouble(1.0)
            # parameters["solver_settings"]["scheme_settings"]["update_relative_residual_norm"].SetDouble(1e-3)
