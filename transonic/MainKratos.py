...
def shock_parameters(mach_infinity,angle_of_attack):
    ...
    cp_min = -0.6
    #     critical_cp = -0.6 / np.sqrt(1-mach_infinity**2)
    # else:  
    #     critical_cp = -0.6 / (np.sqrt(1-mach_infinity**2)-(0.6*(mach_infinity**2/np.sqrt(1-mach_infinity**2))/2)) # -0.6 cp min a cero grados del naca0012
    # Bisection method
    a = 0.2
    b = 0.95
    tol=1.0e-6
    ...
    while True:
        if b - a < tol:
            critical_mach = 0.6
            upwind_factor_constant = 2
            print(mach_infinity,critical_mach)
            #input()
...
if __name__ == "__main__":
    ...
    for mesh_name in mesh_list:
        ...
        for id, mu in enumerate(mu_test):
            ...
            # ax1.plot( x, cp, "o", markersize = 3.0, label = mesh_name + "_" + str(np.round(mu[0] * 180 / np.pi,1))+"º_" + str(np.round(mu[1],2)))
