        ...
        if update_bases or not os.path.exists(f'{mesh_file}/SnapshotsMatrix_FOM.npy'):
            ...
            for i in range(n_clusters):
                ...
                for mu in mu_aux:
                    ...
                    SnapshotsMatrix_c.append(np.load(f'{mesh_file}/Snapshots/{file}'))
                    SnapshotsMatrix_nc.append(np.load(f'{mesh_file}/Snapshots_not_converged_steps/{file}'))
    ...
    def __LaunchROM(self, mu_train_with_indexes, mesh_file, simulation_to_run):
        ...
        for i in range(n_clusters):
            ...
            for mu in mu_aux:
                ...
                file = f'{mu[0]}, {mu[1]}.npy'
                fom  = np.load(f'{mesh_file}/Snapshots/{file}')
