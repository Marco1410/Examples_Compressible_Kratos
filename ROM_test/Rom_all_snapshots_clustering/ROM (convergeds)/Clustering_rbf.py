...
def error_estimation(mu_list, mesh_database):
    ...
    FOM_model = []; RBF_model = []
    for mu in mu_list:
        FOM_model.append(np.load(f'{mesh_database}/Snapshots/{mu[0]}, {mu[1]}.npy'))
...
def error_estimation(mu_train, mu_test, mesh_database):
    if len(mu_train) > 0:
        approximation_error = 0.0
        FOM_model = []; RBF_model = []
        for mu in mu_train:
    ...
    if len(mu_test) > 0:
        ...
        for mu in mu_test:
            FOM_model.append(np.load(f'{mesh_database}/Snapshots/{mu[0]}, {mu[1]}.npy'))
            RBF_model_interpolation.append(np.load(f"{mesh_database}/Interpolated_data/{mu[0]}, {mu[1]}.npy").T)
        FOM_model = np.block(FOM_model)
        ...
        print(f'RBF interpolation approximation error: {approximation_error*100} %')
    return
...
if __name__ == "__main__":
    ...
    for mesh_database in folders:
        ...
        snapshots = np.load(f'{mesh_database}/SnapshotsMatrix_conv.npy')
        ...
        for i, solution in enumerate(training_solutions_list):
            np.save(f"{mesh_database}/Interpolated_data/{mu_train[i][0]}, {mu_train[i][1]}.npy", solution)
        ...
        if plot_custering_info: error_estimation(mu_train, mu_test, mesh_database)   
