...
if __name__ == "__main__":
    ...
    if update_parameters:
        mu_train = np.load('DataBase/mu_train.npy')
        mu_test = np.load('DataBase/mu_test.npy')
        mu_train = [[angle, mach] for [angle, mach] in mu_train if ( 
                        angle <= angle_range[1]
                    and angle >= angle_range[0] 
                    and mach  <=  mach_range[1]
                    and mach  >=  mach_range[0])]
        mu_test = [[angle, mach] for [angle, mach] in mu_test if ( 
                        angle <= angle_range[1]
                    and angle >= angle_range[0] 
                    and mach  <=  mach_range[1]
                    and mach  >=  mach_range[0])]
        
        print(f'Train points: {len(mu_train)}')
        print(f'Test  points: {len(mu_test)}')

        mu_train = list(random.sample(mu_train, number_of_mu_train))
        mu_test  = list(random.sample(mu_test, number_of_mu_test))
