...
def LaunchSimulations(mu_test, mesh_file_name):
    if not os.path.exists(f"Snapshots"):
        os.mkdir(f"Snapshots")
    if not os.path.exists(f"Snapshots_not_converged_steps"):
        os.mkdir(f"Snapshots_not_converged_steps")
    if not os.path.exists(f"FOM_Results"):
        os.mkdir(f"FOM_Results")
    if not os.path.exists(f"FOM_Skin_Data"):
        os.mkdir(f"FOM_Skin_Data")
    if not os.path.exists(f"FOM_Captures"):
        os.mkdir(f"FOM_Captures")
    ...
    for Id, mu in enumerate(mu_test):
        ...
        np.save(f'Snapshots/{mu[0]}, {mu[1]}',BasisOutputProcess._GetSnapshotsMatrix()) 
        np.save(f'Snapshots_not_converged_steps/{mu[0]}, {mu[1]}', np.array(simulation._GetSolver()._GetSolutionStrategy().GetIntermediateSolutionsMatrix()))
...
def CustomizeSimulation(cls, global_model, parameters, mesh_file_name):
    ...
    class CustomSimulation(cls):
        ...
        def FinalizeSolutionStep(self):
            ...
            simulation_name = nametype.removeprefix(f"FOM_Results/")
            skin_data_filename = f"FOM_Skin_Data/{simulation_name}.dat"
            capture_filename = f"FOM_Captures/{simulation_name}.png"
...
def UpdateProjectParameters(parameters, mu=None, mesh_file_name=None):
    ...
    parameters["output_processes"]["vtk_output"][0]["Parameters"]["output_path"].SetString(f'FOM_Results/{angle_of_attack}, {mach_infinity}')
...
if __name__ == '__main__':
    ...
    for mesh_file in mesh_files:
        ...
        if not os.path.exists(f"Snapshots_not_converged_steps"):
            os.mkdir(f"Snapshots_not_converged_steps")
        mu_list = []
        with os.scandir(f'Snapshots') as files:
        ...
        for mu in mu_aux_list:
            file = f'{mu[0]}, {mu[1]}'
            if (   not os.path.exists(f"FOM_Captures/{file}.png")
                or not os.path.exists(f"FOM_Results/{file}")
                or not os.path.exists(f"FOM_Skin_Data/{file}.dat")
                or not os.path.exists(f"Snapshots/{file}.npy")
                or not os.path.exists(f"Snapshots_not_converged_steps/{file}.npy")):
        ...
        for mu in mu_aux_list:
            ...
            snapshotsMatrix.append(np.load(f'Snapshots/{file}'))
            aux_snapshotsMatrix.append(np.load(f'Snapshots_not_converged_steps/{file}'))
