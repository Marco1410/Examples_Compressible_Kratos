    ...
    class CustomSimulation(cls):
        ...
        def InitializeSolutionStep(self):
            ...
            if self._GetSimulationName() == "::[ROM Simulation]:: ":
                ...
                # self._GetSolver()._GetBuilderAndSolver().SetCorrectWithFOM(False)
...
def get_multiple_params_by_Halton_sequence(number_train_values,number_test_values,angle,mach,fix_corners_of_parametric_space):
    ...
    if np.abs(angle[1]-angle[0])< 1e-2:
        ...
        mu_train = []; mu_test = []
        mu_train_rescaled = []; mu_test_rescaled = []
...
if __name__ == "__main__":
    ...
    #CLUSTERING SETTINGS
    clustering_method                  = "k-means" # PEBL - k-means
    n_clusters                         = 1
    ...
    if mu_from_database:
        ...
        if len(data) >= number_of_mu_train:
            mu_train = random.sample(data, number_of_mu_train)
        else:
            print("mu_train size: ", len(data))
            input("pause PRESS to continue")
            mu_train = data
        if len(data) >= number_of_mu_test:
            mu_test  = random.sample(data, number_of_mu_test)
        else:
        ...
        if load_old_mu_parameters:
            ...
            mu_train, mu_test = get_multiple_params_by_Halton_sequence(number_of_mu_train, number_of_mu_test, angle_range, mach_range,
