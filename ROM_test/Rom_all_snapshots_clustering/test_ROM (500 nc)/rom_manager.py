...
from PEBL_RBF import PEBL_RBF_Clustering 
...
def CustomizeSimulation(cls, global_model, parameters):
    ...
    class CustomSimulation(cls):
        ...
        def __init__(self, model,project_parameters):
            ...
            if not self._GetSimulationName() == "Analysis":
        ...
        def Run(self):
            ...
            if self._GetSimulationName() == "Analysis": # FOM
                ...
                self.Initialize()
                # self.model["MainModelPart"].SetBufferSize(3)
                # self._GetSolver()._GetSolutionStrategy().SetKeepSystemConstantDuringIterations(False)
                # self._GetSolver()._GetSolutionStrategy().SetUseOldStiffnessInFirstIterationFlag(False)
                self.RunSolutionLoop()
                self.Finalize()
                for process in self._GetListOfOutputProcesses():
                        if isinstance(process, CalculateRomBasisOutputProcess):
        ...
        def FinalizeSolutionStep(self):
            ...
            if 'Run' not in self.simulation_name:
                if parameters["output_processes"].Has("gid_output"):
                    ...
                    capture_filename   = f"Captures/Cluster_{self.cluster}/{self.simulation_name.split('_')[1]}.png"
                    ...
                    if self._GetSimulationName() == "Analysis": # FOM
                        ...
                        fig.set_figheight(8.0)
                        #### FOM ######
                        fom_simulation_name = self.simulation_name.split('_')[1]
                        fom_simulation_name = f'FOM_{fom_simulation_name}'
                        fom_skin_data_filename = f"Skin_Data/{fom_simulation_name}.dat"
                        ...
                        x  = np.loadtxt(skin_data_filename, usecols=(0,))
                        cp = np.loadtxt(skin_data_filename, usecols=(3,))
                        fig = plt.plot(x, cp, 'xr', markersize = 2.0, label = self.simulation_name.split('_')[0])
                        if np.min(cp) < cp_min:
                            cp_min = np.min(cp)
                        if np.max(cp) > cp_max:
                            cp_max = np.max(cp)
...
def GetRomManagerParameters():
    ...
    general_rom_manager_parameters = KratosMultiphysics.Parameters("""{
            ...
            "ROM":{
                "svd_truncation_tolerance": 1e-15,
            ...
            "HROM":{
                ...
                "element_selection_svd_truncation_tolerance": 1e-15,
...
if __name__ == "__main__":
    ...
    # if not set_train_rom and set_test_rom:
    #     update_clusters = True
    #     _,mu_test,_,aux_mu_test = load_mu_parameters()
