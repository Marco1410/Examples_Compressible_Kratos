...
def PEBL_RBF_Clustering( plot_custering_info      = True,
                         plot_clusters            = True,
    ...
    snapshots  = np.load(f'DataBase/SnapshotsMatrix_conv.npy')
    ...
    training_solutions_list = [rom.predict([element]).snapshots_matrix for element in mu_train_not_scaled]

    for i, solution in enumerate(training_solutions_list):
        np.save(f"RBF_interpolated_data/{mu_train[i][0]}, {mu_train[i][1]}.npy", solution)
...
def PEBL_RBF_Clustering_Prediction( plot_custering_info      = True,
    ...
    if len(mu_test) > 0:
        ...
        for i, solution in enumerate(interpolated_solutions_list):
            np.save(f"RBF_interpolated_data/{mu_test[i][0]}, {mu_test[i][1]}.npy", solution)
...
def PEBL_error_estimation(mu_train, mu_test):
    if len(mu_train) > 0:
        ...
        for mu in mu_train:
            ...
            RBF_model.append(np.load(f"RBF_interpolated_data/{mu[0]}, {mu[1]}.npy").T)
    ...
    if len(mu_test)>0 and os.path.exists(f'RBF_interpolated_data/{mu_test[0][0]}, {mu_test[0][1]}.npy'):
        approximation_error = 0.0
        FOM_model = []; RBF_model_interpolation = []
        for mu in mu_test:
            FOM_model.append(np.load(f'DataBase/Snapshots/{mu[0]}, {mu[1]}.npy'))
            RBF_model_interpolation.append(np.load(f"RBF_interpolated_data/{mu[0]}, {mu[1]}.npy").T)
        FOM_model = np.block(FOM_model)
...
if __name__ == "__main__":
    ...
    PEBL_RBF_Clustering( bisection_tolerance = bisection_tolerance, 
    ...
    predicted_indexes_list = PEBL_RBF_Clustering_Prediction(    bisection_tolerance      = bisection_tolerance, 
                                                                POD_tolerance            = POD_tolerance, 
                                                                plot_clusters_prediction = True,
                                                                plot_custering_info      = True,
                                                                mu_train                 = mu_train,
