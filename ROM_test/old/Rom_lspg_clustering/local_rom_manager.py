...
class LocalRomManager(object):
    ...
    def __LaunchTrainROM(self, mu_train_with_indexes, mesh_file, update_bases):
        if update_bases or not os.path.exists(f'{mesh_file}/SnapshotsMatrix_FOM.npy'):
            KratosMultiphysics.kratos_utilities.DeleteDirectoryIfExisting(f"{mesh_file}")
            ...
            for i in range(n_clusters):
                ...
                for mu in mu_aux:
                    ...
                    SnapshotsMatrix.append(np.load(f'DataBase/{mesh_file}/Snapshots/{file}'))
                    SnapshotsMatrix_c.append(np.load(f'DataBase/{mesh_file}/Snapshots/{file}'))
                    SnapshotsMatrix_nc.append(np.load(f'DataBase/{mesh_file}/Snapshots_not_converged_steps/{file}'))
                ...
                NumberOfRomModes_list.append(BasisOutputProcess.GetNumberOfRomModes())
    ...
    def __LaunchROM(self, mu_train_with_indexes, mesh_file, simulation_to_run):
        ...
        for i in range(n_clusters):
            ...
            for mu in mu_aux:
                ...
                file = f'{mu[0]}, {mu[1]}.npy'
                fom  = np.load(f'DataBase/{mesh_file}/Snapshots/{file}')
                ...
                rom_info_steps_list.append([i,
                                            ...
                                            np.linalg.norm(fom-rom)*100/np.linalg.norm(fom),
                                            ])
