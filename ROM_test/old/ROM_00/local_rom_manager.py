...
class RomManager(object):
    ...
    def __LaunchTrainROM(self, mu_train):
        ...
        LocalSnapshotsMatrix = []; LocalSnapshotsMatrix_nc = []; SnapshotsMatrix = []
        for Id, mu in enumerate(mu_train):
            ...
            # number_of_last_snapshots = 10
            # if number_of_last_snapshots > np.load(f'DataBase/Snapshots_not_converged/{file}').shape[1]: 
            #     LocalSnapshotsMatrix_nc.append(np.load(f'DataBase/Snapshots_not_converged/{file}'))
            # else:    
            #     LocalSnapshotsMatrix_nc.append(np.load(f'DataBase/Snapshots_not_converged/{file}')[:,-number_of_last_snapshots:])
        ...
        # local_phi_c,_,_,_= RandomizedSingularValueDecomposition().Calculate(LocalSnapshotsMatrix, 1e-12)

        # local_S_nc = LocalSnapshotsMatrix_nc
        # for k in range(10):
        #     local_S_nc = local_S_nc - (local_phi_c @ (local_phi_c.T @ local_S_nc))

        # local_S_nc = (local_phi_c @ (local_phi_c.T @ local_S_nc))
                
        # local_phi_nc,_,_,_= RandomizedSingularValueDecomposition().Calculate(local_S_nc, 1e-12)
