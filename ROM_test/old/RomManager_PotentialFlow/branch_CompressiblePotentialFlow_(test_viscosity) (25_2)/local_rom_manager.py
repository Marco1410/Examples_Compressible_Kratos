...
class LocalRomManager(object):
    ...
    def __LaunchTrainROM(self, mu_train):
        ...
        if not os.path.exists("DataBase"):
            os.mkdir("DataBase")
            os.mkdir("DataBase/Gid_Results")
            os.mkdir("DataBase/Data")
            os.mkdir("DataBase/NonSharpSolution")
        ...
        for Id, mu in enumerate(mu_train):
            ...
            if os.path.exists("DataBase/Gid_Results/" + str(mu[0]) + ", " + str(mu[1]) + ".post.bin"):
                parameters_copy["output_processes"].RemoveValue("gid_output")
            else:
                parameters_copy["output_processes"]["gid_output"][0]["Parameters"]["output_name"].SetString("DataBase/Gid_Results/"+ str(mu[0]) + ", " + str(mu[1]))
            ...
            non_sharp_case_name = "DataBase/NonSharpSolution/"+ str(mu[0]) + ", " + str(mu[1]) +".npy"
            if os.path.exists(non_sharp_case_name):
                aux_non_sharp_snapshotmatrix = np.load(non_sharp_case_name)
                SnapshotsMatrix.append(aux_non_sharp_snapshotmatrix)
            else:
                ...
                aux_analysis_stage_class = self._GetAnalysisStageClass(parameters_copy)
                aux_simulation = self.CustomizeSimulation(aux_analysis_stage_class,aux_model,parameters_copy)
                aux_simulation.Run()
                for process in aux_simulation._GetListOfOutputProcesses():
                    if isinstance(process, CalculateRomBasisOutputProcess):
                        aux_BasisOutputProcess = process
                aux_non_sharp_snapshotmatrix = aux_BasisOutputProcess._GetSnapshotsMatrix()
                np.save(non_sharp_case_name, aux_non_sharp_snapshotmatrix)
                SnapshotsMatrix.append(aux_non_sharp_snapshotmatrix)
        ...
        np.save("test/snapshotsmatrix", SnapshotsMatrix)
