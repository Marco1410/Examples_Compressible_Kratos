        ...
        for Id, mu in enumerate(mu_train):
            ...
            if use_uncorrected_solutions:
                ...
                if os.path.exists(Uncorrected_case_name):
                    ...
                    critical_mach               = parameters_copy["solver_settings"]["scheme_settings"]["critical_mach"].GetDouble()
                    upwind_factor_constant      = parameters_copy["solver_settings"]["scheme_settings"]["upwind_factor_constant"].GetDouble()
                    update_transonic_tolerance  = 1e-30
                    parameters_copy["solver_settings"]["scheme_settings"]["update_critical_mach"].SetDouble(critical_mach)
                    parameters_copy["solver_settings"]["scheme_settings"]["update_upwind_factor_constant"].SetDouble(upwind_factor_constant)
                    parameters_copy["solver_settings"]["scheme_settings"]["update_transonic_tolerance"].SetDouble(update_transonic_tolerance)
