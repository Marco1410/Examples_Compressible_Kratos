...
class RomManager(object):
    ...
    def __init__(self,project_parameters_name, general_rom_manager_parameters, CustomizeSimulation, UpdateProjectParameters,UpdateMaterialParametersFile):
        ...
        self.ROMvsFOM_train = 0
        self.ROMvsHROM_train = 0
        self.ROMvsFOM_test = 0
        self.ROMvsHROM_test = 0
    ...
    def Fit(self, mu_train=[None], store_all_snapshots=False, store_fom_snapshots=False, store_rom_snapshots=False, store_hrom_snapshots=False, store_residuals_projected = False):
        ...
        return self.ROMvsFOM_train, self.ROMvsHROM_train



    def Test(self, mu_test=[None], store_all_snapshots=False, store_fom_snapshots=False, store_rom_snapshots=False, store_hrom_snapshots=False, store_residuals_projected = False):
        if len(mu_test) > 0:
            chosen_projection_strategy = self.general_rom_manager_parameters["projection_strategy"].GetString()
            ...
            if chosen_projection_strategy == "galerkin":
                if any(item == "ROM" for item in testing_stages):
                    ...
                    if store_all_snapshots or store_fom_snapshots:
                        self._StoreSnapshotsMatrix('fom_snapshots_test', fom_snapshots)
                    self._ChangeRomFlags(simulation_to_run = "GalerkinROM")
                    rom_snapshots = self.__LaunchTestROM(mu_test)
                    if store_all_snapshots or store_rom_snapshots:
                        self._StoreSnapshotsMatrix('rom_snapshots_test', rom_snapshots)
                ...
                if any(item == "HROM" for item in testing_stages):
                    ...
                    if store_all_snapshots or store_hrom_snapshots:
                        self._StoreSnapshotsMatrix('hrom_snapshots_test', hrom_snapshots)
        ...
        return self.ROMvsFOM_test, self.ROMvsHROM_test
    ...
    def __LaunchTrainROM(self, mu_train):
        ...
        LocalSnapshotsMatrix = []; LocalSnapshotsMatrix_nc = []; SnapshotsMatrix = []; GlobalSnapshotsMatrix_nc = []
        for Id, mu in enumerate(mu_train):
            ...
            LocalSnapshotsMatrix.append(np.load(f'DataBase/Snapshots/{file}'))
            LocalSnapshotsMatrix_nc.append(np.load(f'DataBase/Snapshots_not_converged/{file}'))
        
        GlobalSnapshotsMatrix_nc.append(np.load(f'DataBase/SnapshotsMatrix_not_conv.npy'))

        SnapshotsMatrix = np.block(SnapshotsMatrix)
        LocalSnapshotsMatrix = np.block(LocalSnapshotsMatrix)
        LocalSnapshotsMatrix_nc = np.block(LocalSnapshotsMatrix_nc)
        GlobalSnapshotsMatrix_nc = np.block(GlobalSnapshotsMatrix_nc)

        # print(f'Converged matrix shape: {LocalSnapshotsMatrix.shape}')
        # print(f'Not converged matrix shape: {LocalSnapshotsMatrix_nc.shape}')
        # input()

        local_phi_c,_,_,_= RandomizedSingularValueDecomposition().Calculate(LocalSnapshotsMatrix, 1e-15)
        local_S_nc = LocalSnapshotsMatrix_nc
        for k in range(10):
            local_S_nc = local_S_nc - (local_phi_c @ (local_phi_c.T @ local_S_nc))
        local_phi_nc,_,_,_= RandomizedSingularValueDecomposition().Calculate(local_S_nc, 1e-6)

        phi_c = np.linalg.svd(np.c_[local_phi_c, local_phi_nc], full_matrices=False)[0]

        # S_nc = GlobalSnapshotsMatrix_nc
        # for j in range(10):
        ...
        BasisOutputProcess._PrintRomBasis(phi_c)  
