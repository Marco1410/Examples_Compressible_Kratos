...
class LocalRomManager(object):
    ...
    def __LaunchTrainROM(self, mu_train, use_uncorrected_solutions):
        ...
        if not os.path.exists("DataBase"):
            ...
            os.mkdir("DataBase/UncorrectedSolutions/Gid_Results")
        ...
        for Id, mu in enumerate(mu_train):
            ...
            if use_uncorrected_solutions:
                ...
                if os.path.exists(Uncorrected_case_name):
                    ...
                    if os.path.exists("DataBase/UncorrectedSolutions/Gid_Results/" + str(mu[0]) + ", " + str(mu[1]) + ".post.bin"):
                        aux_parameters["output_processes"].RemoveValue("gid_output")
                    else:
                        aux_parameters["output_processes"]["gid_output"][0]["Parameters"]["output_name"].SetString("DataBase/UncorrectedSolutions/Gid_Results/"+ str(mu[0]) + ", " + str(mu[1]))
