...
class LocalRomManager(object):
    ...
    def Fit(self, mu_train=[None], use_non_linear_steps=False):
        mu_train_errors = []
        if (len(mu_train) > 0):
            ...
            if chosen_projection_strategy == "galerkin":
                if any(item == "ROM" for item in training_stages):
                    fom_snapshots = self.__LaunchTrainROM(mu_train, use_non_linear_steps)
    ...
    def __LaunchTrainROM(self, mu_train, use_non_linear_steps):
        """
        ...
        SnapshotsMatrix = []; error_SnapshotsMatrix = []
        for Id, mu in enumerate(mu_train):
            ...
            if use_non_linear_steps:
                ...
                if not os.path.exists(case_name) or not os.path.exists(non_linear_steps_case_name):
                    ...
                    SnapshotsMatrix.append(np.array(simulation._GetSolver()._GetSolutionStrategy().GetIntermediateSolutionsMatrix()))
                    ...
                    error_SnapshotsMatrix.append(BasisOutputProcess._GetSnapshotsMatrix())
                else:
                    ...
                    error_SnapshotsMatrix.append(case)
            else:
                case_name = "DataBase/"+ str(mu[0]) + ", " + str(mu[1]) +".npy"
        ...
        print("SnapshotsMatrix size: ", SnapshotsMatrix.shape)
        ...
        np.save("rom_data/M_full.npy",SnapshotsMatrix)
        np.save("rom_data/M_simple.npy",error_SnapshotsMatrix) 
        u, s, Vt = np.linalg.svd(error_SnapshotsMatrix, full_matrices=False)
        np.save("rom_data/u_simple.npy",u)
        np.save("rom_data/s_simple.npy",s)
        np.save("rom_data/v_simple.npy",Vt)
        input(":::::::::::::::::::::::")
