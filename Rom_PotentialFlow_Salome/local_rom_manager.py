...
class LocalRomManager(object):
        ...
        if (len(mu_train) > 0):
            ...
            if chosen_projection_strategy == "galerkin":
                if any(item == "ROM" for item in training_stages):
                    ...
                    np.save("ROMvsFOM_error_train_per_node", (fom_snapshots - rom_snapshots))
                    self.ROMvsFOM_train_per_node_average = np.linalg.norm(fom_snapshots - rom_snapshots)/ fom_snapshots.size
        ...
        if (len(mu_test) > 0):
            ...
            if chosen_projection_strategy == "galerkin":
                if any(item == "ROM" for item in testing_stages):
                    ...
                    np.save("ROMvsFOM_error_test_per_node", (fom_snapshots - rom_snapshots)/ fom_snapshots.size)
                    self.ROMvsFOM_test_per_node_average = np.linalg.norm(fom_snapshots - rom_snapshots)/ fom_snapshots.size
    ...
    def PrintErrors(self):
        ...
        if any(item == "ROM" for item in training_stages) and self.ROMvsFOM_train != 0:
            ...
            print("average approximation error per node in train set FOM vs ROM: ", self.ROMvsFOM_train_per_node_average," %")
        ...
        if any(item == "ROM" for item in testing_stages) and self.ROMvsFOM_test != 0:
            ...
            print("average approximation error per node in test set FOM vs ROM: ", self.ROMvsFOM_test_per_node_average," %")
